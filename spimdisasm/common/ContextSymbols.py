...
class SymbolBindingType(enum.Enum):
    LOCAL           = 0
    GLOBAL          = 1
    WEAK            = 2
    ...
    def toStr(self) -> str:
        try:
            ...
            return "GLOBAL" # Default symbol binding to global

    @staticmethod
    def fromValue(value: int) -> SymbolBindingType|None:
        try:
            return SymbolBindingType(value)
        except ValueError:
            return None
...
class ElfSymbolType(enum.Enum):
    ...
    def toStr(self) -> str:
        try:
            ...
            # should this return NOTYPE?
            return "NOTYPE" # Default symbol type to NOTYPE

    @staticmethod
    def fromValue(value: int) -> ElfSymbolType|None:
        try:
            return ElfSymbolType(value)
        except ValueError:
            return None
...
class ContextSymbol:
    ...
    userDeclaredBind: int|None = None # Symbol binding: Weak/Global/Local
    ...
    elfSymbolType: ElfSymbolType|None = None
    ...
    def getBindName(self) -> str:
        if self.userDeclaredBind == None:
            return "global"
        return f"{SymbolBindingType(self.userDeclaredBind).toStr().lower()}"
    
    def getElfSymbolType(self) -> ElfSymbolType:
        return self.elfSymbolType
    
    def getElfSymbolTypeName(self) -> str:
        return f"{ElfSymbolType(self.elfSymbolType).toStr().lower()}"
...
=======
        # output += f"{self.firstLoAccess},{self.isAutogeneratedPad()},{self.isElfNotype},"
        output += f"{self.firstLoAccess},{self.isAutogeneratedPad()},{self.elfSymbolType.toStr() == 'NOTYPE'},"
>>>>>>> Stashed changes
